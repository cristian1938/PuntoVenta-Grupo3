...
MessageBox.Show("INGRESA UN PRODUCTO");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string numeroboleta = autoincrementableBOLETA();
                for (int i = 0; i < dgvVentas.RowCount; i++)
                {
                    string Consulta2 = "insert into TBoleta values ('" + numeroboleta + "','" + txtCodVentas.Text + "','"
                            + dgvVentas[1, i].Value.ToString() + "','" + dgvVentas[3, i].Value.ToString() + "')";

                    EjecutarSelect(Consulta2);
                }
                ...
                FrmBoleta a = new FrmBoleta();

                for (int i = 0; i < dgvVentas.RowCount; i++)
                {
                    a.dgvVentas.Rows.Add(dgvVentas[1, i].Value.ToString(), dgvVentas[2, i].Value.ToString(), dgvVentas[3, i].Value.ToString());
                }

                dgvVentas.Rows.Clear();
                a.txtsubtotal.Text = txtsubtotal.Text;
                a.txtigv.Text = txtigv.Text;
                a.txtTotal.Text = txttotal.Text;
                a.txtboleta.Text = numeroboleta;
                a.ShowDialog();
            ...
            {
                MessageBox.Show("INGRESE PRODUCTOS PARA REALIZAR LA VENTA");
            }
        }
        ...
        public string autoincrementableBOLETA()
        {
            String A;
            //b0001
            A = ultimoValorAtributoBOLETA();
            string cadena="";
            string b = A.Substring(0, 5);
            double d = double.Parse(A.Substring(2, 4));
            d = d + 1;
        ...
        public string ultimoValorAtributoBOLETA()
        {//-- Recupera el valor de un atributo del dataset
            string A;

            string Consulta = "select MAX(CodBoleta) AS  ULTIMO  from TBoleta";
            EjecutarSelect(Consulta);
            A = ValorAtributo("ULTIMO");
            if (A == "NULL")
            {
